...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-4 sm:px-6 pt-20">
      {/* Ambient glow orbs */}
      <div className="absolute top-1/4 left-1/4 w-[250px] sm:w-[500px] h-[250px] sm:h-[500px] rounded-full opacity-[0.04] blur-[80px] sm:blur-[120px]" style={{ background: 'hsl(165 80% 48%)' }} />
      <div className="absolute bottom-1/4 right-1/4 w-[200px] sm:w-[400px] h-[200px] sm:h-[400px] rounded-full opacity-[0.03] blur-[60px] sm:blur-[100px]" style={{ background: 'hsl(190 90% 50%)' }} />
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center w-full">
        <motion.div
          ...
          className="inline-flex items-center gap-2.5 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-xs sm:text-sm text-primary mb-8 sm:mb-10 backdrop-blur-sm"
        >
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-6xl md:text-8xl font-extrabold tracking-tight leading-[1.05] mb-5"
        >
          Full Stack
          <br />
        ...
        <motion.p
          ...
          className="text-xs sm:text-sm text-muted-foreground flex items-center justify-center gap-2 mb-10 px-2 text-center leading-relaxed"
        >
        ...
        <motion.div
          ...
          className="grid grid-cols-3 gap-3 sm:gap-6 max-w-xs sm:max-w-lg mx-auto pb-10"
        >
          {stats.map((stat, i) => (
            <motion.div
              key={stat.label}
              ...
              className="text-center glass-card p-3 sm:p-4 rounded-2xl"
            >
              <div className="text-xl sm:text-2xl md:text-3xl font-bold text-gradient">
                <AnimatedCounter raw={stat.value} delay={(0.7 + i * 0.1) * 1000} />
              </div>
              <div className="text-[9px] sm:text-[11px] text-muted-foreground mt-1 sm:mt-1.5 font-medium tracking-wide">{stat.label}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
